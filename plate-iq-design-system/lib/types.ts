...
export interface Dish { id:string; name:string; category:string; forecast:number; lowerBound:number; upperBound:number; confidence:number; actualOrders:number; prepared:number; initialBatch:number; nextBatch:number; batchSize:number; leadTimeMinutes:number; status:DishStatus; wasteKg:number; wasteCost:number; ingredients:Record<string,number> }
export interface Forecast { id:string; dishId:string; period:string; forecast:number; lowerBound:number; upperBound:number; confidence:number; actual:number }
...
export interface InventoryItem { id:string; name:string; unit:string; currentStock:number; reorderPoint:number; dailyUsage:number; unitCost:number; status:'Healthy'|'Low'|'Critical'; daysLeft:number; trend:'Rising'|'Stable'|'Falling' }
export interface WasteRecord { id:string; dishId:string; prepared:number; consumed:number; spoilageKg:number; overproductionKg:number; wasteKg:number; wasteCost:number; date:string }
